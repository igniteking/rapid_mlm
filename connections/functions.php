...
function Register($user_type, $username, $password, $r_pswd, $conn, $email, $refral, $location)
{
                    ...
                    if (preg_match("/[A-Z]/", $password)) {
                        if (preg_match("/[a-z]/", $password)) {
                            if (preg_match("/\W/", $password)) {
                                ...
                                $code = substr(str_shuffle(str_repeat($x = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ', ceil(5 / strlen($x)))), 1, 5);
                                if ($refral == 00000) {
                                    mysqli_query($conn, "INSERT INTO `user_data`(`id`, `user_name`, `user_email`, `user_password`, `user_type`, `created_at`, `user_code`) VALUES 
                                    (NULL,'$username','$email','$hashedPwd','$user_type', '$date', '$code')");
                                } else {
                                    $date = date("Y-m-d H:s:i");
                                    $parent_id = array_values(mysqli_fetch_array($conn->query("SELECT `id` from `user_data` WHERE user_code='$refral'")))[0];
                                    mysqli_query($conn, "INSERT INTO `user_data`(`id`, `user_name`, `user_email`, `user_password`, `user_type`, `created_at`, `user_code`) VALUES 
                                 (NULL,'$username','$email','$hashedPwd','$user_type', '$date', '$code')");
                                    $child_id = array_values(mysqli_fetch_array($conn->query("SELECT `id` from `user_data` WHERE user_code='$code'")))[0];
                                    mysqli_query($conn, "INSERT INTO `mlm_tree`(`id`, `parent_id`, `child_id`, `child_location`, `created_at`) VALUES (NULL,'$parent_id','$child_id','$location','$date')");
                                }
...
function uploadImage($filename)
{
    ...
    $folder = mkdir("./extra/$random");
    $target_dir = "./extra/$random/";
